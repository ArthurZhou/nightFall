const audioFile = "extern/audio.mp3"
const coverFile = "extern/cover.jpg"
let lrcFile = 'extern/lyrics.lrc'

// initialize global var
let playing = false // is playing
let firstPlay = true // is first time playing
let mouseDown = false
let scrollEvent = 0

let lrcs = [] // lyrics pool  e.g. {
//                                     0: {
//                                            t: '00:00.00'
//                                            l: 'Ooo~ Ooo~ Let's sing the song'
//                                        }
//                                 }

var currentLrc = 0 // set to -1 if music is over
let highlightLrc = []

const btn = document.querySelector('.play') // get play/pause button
const progress = document.querySelector('.progress').children[0] // progress bar
const canvas = document.querySelector("#fft") // get canvas
const lrcContainer = document.querySelector('.lyrics')
...
function initAudio() {
    ...
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)() // create new audio context

    audioSource = audioCtx.createMediaElementSource(audio)
    analyser = audioCtx.createAnalyser()
    audioSource.connect(analyser) // connect audio to fft
    analyser.connect(audioCtx.destination) // connect audio to speaker
    analyser.fftSize = 256
    bufferLength = analyser.frequencyBinCount
    dataArray = new Uint8Array(bufferLength)
    barWidth = canvas.width / bufferLength
    debug(`FFT initialized. h:w:${canvas.height}:${canvas.width}  size:${analyser.fftSize}`)
    debug(`Audio sample rate: ${analyser.context.sampleRate}hz`)
}

function initInfo() {
    // load cover
    document.querySelector('#cover').src = coverFile

    // try to load cover img. if failed, use a place holder
    document.querySelector('#cover').addEventListener("load", () => {
        debug('Cover img loaded.')
    })
    document.querySelector('#cover').addEventListener("error", () => {
        warn('Failed to load cover img, using fallback.')
        document.querySelector('#cover').src = 'assets/placeholder.svg'
    })

    // title and author
    ID3.loadTags(audioFile, function() {
        debug('Retrieving tags from audio container...')
        let tags = ID3.getAllTags(audioFile)
        document.querySelector('.title').innerHTML = tags.title
        document.querySelector('.artist').innerHTML = '&#9632; ' + tags.artist
        debug(`Audio tags retrieved. Title:'${tags.title}  Artist:'${tags.artist}`)
    })
}

function initCtrl() {
    btn.addEventListener('click', function() { // bind play/pause button
        debug('Initializing play/pause button...')
        if (playing == false) {
            play()
        } else {
            pause()
        }
        debug('Play/pause button ready!')
    })

    // when metadata is loaded
    audio.onloadedmetadata = function() {
        debug(`Audio loaded. duration: ${getMMSS(audio.duration)}  initializing progress bar...`)
        // progress
        progress.max = audio.duration // max value of progress bar == audio duration
        document.querySelector('.total').innerHTML = getMMSS(audio.duration) // human readable duration under progress bar
        setInterval(() => {
            ...
            if (audio.duration == audio.currentTime && playing == true) { // when playback finished
                debug('Audio playback finished.')
                currentLrc = -1
                for (i=0;i<highlightLrc.length;i++) {
                    highlightLrc[i].classList.remove('highlighted')
                }
                pause()
            }
    ...
    progress.addEventListener('mousedown', function() { // when mouse pressed down on progress bar, start dragging
        mouseDown = true
        debug('Mouse button pressed down.')
    })
    ...
    progress.addEventListener('mouseup', function() { // when mouse released on progress bar, stop dragging
        ...
        turnLyrics(audio.currentTime) // scroll lyrics to current audio time
        debug(`Changing audio play time to ${getMMSS(audio.currentTime)}`)
    })
    ...
    progress.addEventListener('mouseleave', function() { // when mouse leave progress bar, stop dragging
        if (mouseDown) { // only handle this event when mouse pressed down on bar and leave without a 'mouse up'
            ...
            turnLyrics(audio.currentTime)
            debug(`Changing audio play time to ${getMMSS(audio.currentTime)}`)
        }
    })

    debug('Audio progress bar ready!')
}
...
function pause() {
    ...
    audio.pause()
    ctx.clearRect(0, 0, canvas.width, canvas.height) // clear fft canvas
}
...
function initLyrics() {
    ...
    .then(function(response) {
        response.text().then(function(text) {
            // lyric e.g.   [00:00.00]Ooo~ Ooo~ Let's sing the song
            //              ^--time--^-----lyric-------------------
            //then drop this^        ^do the split here first

            let rawLrc = text.split('\n') // split lyrics by line breaks
            for (i=0;i<rawLrc.length;i++) {
                let processor = rawLrc[i].split(']')
                let time = processor[0].slice(1, 9)
                let lyric = processor[1]
                if (time != undefined && lyric != undefined) {
                    lrcs.push({t:time, l:lyric})
                    lrcContainer.innerHTML += `<li id="lrc_${time}" class>${lyric}</li>`
                }
            }
        })
    })
    ...
    debug('Lyrics ready!')
    setInterval(showLyrics, 10) // update lyrics every 0.01s
}

function showLyrics() {
    if (currentLrc != -1) { // check if music is over
        if (audio.currentTime >= getSeconds(lrcs[currentLrc]['t'])) {
            for (i=0;i<highlightLrc.length;i++) {
                highlightLrc[i].classList.remove('highlighted')
            }
            let currentLrcTxt = document.getElementById(`lrc_${lrcs[currentLrc]['t']}`)
            currentLrcTxt.classList.add('highlighted')
            highlightLrc.push(currentLrcTxt)
            let list = document.querySelector('.lyrics')
            let lineHeight = list.children[0].getBoundingClientRect().height
            let lineNum = Math.round(list.clientHeight / lineHeight)
            let v = currentLrc*lineHeight-lineHeight*Math.round(lineNum/2)
    ...
    if (currentLrc >= lrcs.length) {
        currentLrc = -1
    }
}

function turnLyrics(time) {
    for (i=0;i<lrcs.length;i++) {
        let lrcTime1 = getSeconds(lrcs[i]['t'])
        let lrcTime2
        if (lrcs[i+1] != undefined) { //this checks if we reached the end of lyrics
            lrcTime2 = getSeconds(lrcs[i+1]['t'])
        } else {
            lrcTime2 = audio.length
        }
        
        if (time >= lrcTime1 && time < lrcTime2) {
            currentLrc = i
            debug(`Turning lyrics to line ${currentLrc}  Music time:${time} line1:${lrcTime1} line2:${lrcTime2}`)
            break
        }
    }
}
...
debug('Initializing audio related stuffs...')
initInfo()
initCtrl()
initLyrics()
debug('Audio ready!')